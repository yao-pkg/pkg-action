...
export function crossCompileRisk(host: Target, target: Target): string | null {
  ...
  if (target.os === 'linux' && target.arch === 'arm64' && host.arch !== 'arm64') {
    return 'Linux-arm64 cross-compile from a non-arm64 host hits the pkg bytecode fabricator bug (#87/#181) on Node 22. Use a linux/arm64 runner or enable fallback-to-source in your pkg config.';
  }
  // Non-Windows → win-x64 on Node 22 also hits the fabricator bug.
  if (target.os === 'win' && target.arch === 'x64' && host.os !== 'win') {
    return 'win-x64 cross-compile from a non-Windows host hits the pkg bytecode fabricator bug (#87/#181) on Node 22. Use a windows runner or enable fallback-to-source in your pkg config.';
  }
