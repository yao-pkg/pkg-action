...
in this repo. All effectful operations (exec, fs, network) are injected so
tests can swap in doubles.

| Module                      | Purpose                                                                       |
| --------------------------- | ----------------------------------------------------------------------------- |
| `errors.ts`                 | Domain errors (`ValidationError`, `SignError`, `UploadError`, `ArchiveError`) |
...
| `uploader.ts`               | `createDefaultArtifactUploader`, `createDefaultReleaseAttacher`               |
...
| `release-body.ts`           | `buildReleaseBody` markdown assembly                                          |
| `sbom.ts`                   | CycloneDX 1.5 + SPDX 2.3 emission from dep tree                               |
| `distribution.ts`           | Homebrew formula + Scoop manifest renderers + GitHub PR publisher             |
| `docker.ts`                 | Multi-arch OCI build + push + manifest list                                   |
...
## 3. Orchestrator pipeline (`packages/build/src/main.ts`)
...
writeSbom                 → CycloneDX | SPDX (when performance.sbom != 'none')
uploadArtifacts           → @actions/artifact, unique name per target
attacher.attach           → release attach (when attach-to-release)
publishHomebrew           → open PR against tap repo (when homebrew-tap-repo)
publishScoop              → open PR against bucket repo (when scoop-bucket-repo)
publishDocker             → per-arch build + push + manifest list (when docker-image)
writeSummary              → GITHUB_STEP_SUMMARY markdown
setOutputs                → binaries / artifacts / checksums / version / release-url
```

All heavy lifting lives in `@pkg-action/core`; the orchestrator is a wiring
shell around the ExecFn bridge (`getExecOutput` from `@actions/exec`) and the
Octokit-backed publishers.

## 4. Dependency-injection pattern

No OOP. Every effectful boundary is a `readonly` interface with a `createDefault*`
factory that wires the production implementation. Tests construct fakes inline:

```ts
// Production
const publisher = createDefaultDockerPublisher({ exec: execBridge, logger });
...
| Boundary        | Interface               | Default factory                                       |
| --------------- | ----------------------- | ----------------------------------------------------- |
| Process exec    | `ExecFn`                | `createActionsExecBridge()` (bridges `@actions/exec`) |
| Logging         | `Logger`                | `actionsLogger` (bridges `@actions/core` annotations) |
| Artifact upload | `ArtifactUploader`      | `createDefaultArtifactUploader()`                     |
| Release attach  | `ReleaseAttacher`       | `createDefaultReleaseAttacher(token, opts)`           |
| Distribution PR | `DistributionPublisher` | `createDefaultDistributionPublisher({ token })`       |
| Docker publish  | `DockerPublisher`       | `createDefaultDockerPublisher({ exec, logger })`      |

The `{ token }` / `{ exec, logger }` shape is kept parallel so callers can read
a single mental model.

## 5. Codegen — `scripts/gen-action-yml.ts`
...
**CI gate**: `ci.yml` runs `yarn gen` + `git diff --exit-code` — a missing
...
workflow_dispatch. Matrix jobs:

- tiny-app-cjs & tiny-app-esm round-trip on ubuntu / macos / windows
...
- provenance (`attest-build-provenance@v4` + `gh attestation verify --repo`)
- sbom-cyclonedx + sbom-spdx
- docker-publish (ghcr.io push + pull + run + grep version line)
...
- **Total**: 296 unit tests, ~95% line coverage at 2026-04-23.
...
7. Replace `STATUS.yaml` with `CHANGELOG.md` covering M0 → M6.

## 11. Known architectural debt

Tracked with owner-visible references in `STATUS.yaml#pending.architecture`:

- **Orchestrator size** — `main.ts` is ~780 lines. Target refactor extracts
  `finalizeBinary`, `createActionsExecBridge`, and `publishers.ts` into core.
- **Core barrel cost** — every test pays Octokit + resedit import cost. Subpath
  exports (`@pkg-action/core/signing` etc.) would let tests scope imports.

Everything else listed under `pending` is **test-surface** (more fixtures, live
credentials) rather than code correctness.
